...
const Preferences = () => {
  ...
  const changePassword = () => {
    ...
    const response = axios.put("http://localhost:8080/api/supplier/me", JSON.stringify(body),{
      headers : {
  ...
  React.useEffect(()=>{
    const response = axios.get("http://localhost:8080/api/supplier/me", {
      headers : {
        "Content-Type" : "application/json",
        "Authorization" : `Bearer ${localStorage.getItem("jwt")}`,
        "Access-Control-Allow-Origin" : true
    }}).then(response => {
      setLang(response.data.supplier.lang);
    })
      ...
      <Stack sx={{display:'flex', width:"100%"}} alignItems='center'>
        ...
        <Box sx={{borderRadius:"16px", backgroundColor:"white", width:"80%", mt:5}}>
          <Stack alignItems='center' justifyContent={"center"} sx={{m:4}}>
            ...
            <Stack direction='row' alignItems='center' justifyContent='center' sx={{mt:2, width: 'auto'}}>
